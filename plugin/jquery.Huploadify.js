(function($){
   //如扩展$.fn.abc(),即$.fn.abc()是对jquery扩展了一个abc方法,那么后面你的每一个jquery实例都可以引用这个方法了$("#div").abc();
	$.fn.Huploadify = function(opts){
		//itemTemp 生成一个上传单个文件的模板
		var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="clear"><span class="float_l up_filename">${fileName}</span>'+
		'<a href="javascript:void(0);" class="uploadbtn">上传</a>'+
		'<a href="javascript:void(0);" class="delfilebtn float_r">删除</a>'+
		'</div><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div></div>';
		var defaults = {
			...
			fileSizeLimit:9999999999999999999999999999,//允许上传的文件大小，单位KB
				...
				_filter: function(files) {		//选择文件组的过滤方法
					...
					if(typeArray.length>0){
						for(var i=0,len=files.length;i<len;i++){
							var f = files[i];
							if(parseInt(F.formatFileSize(f.size,false))<=option.fileSizeLimit){
						...
						xhr.onreadystatechange = function(e) {
							if(xhr.readyState == 4){
								if(xhr.status == 200){
									...
									//console.log("上传失败",option.onUploadError);
								}
